...
export class VesselLocationBased extends DeliveryAutocompleteComponent
  implements OnInit{
  ...
  @Input('model') set _setFormValues(formValues) { 
    ...
    if (this.formValues.locationDiscountRules && this.formValues.locationDiscountRules.length) {
      this.formatValues();
    }
  }
  ...
  formatValues() {
    for (let i = 0; i < this.formValues.locationDiscountRules.length; i++) {
      if (this.formValues.locationDiscountRules[i].amount) {
        this.formValues.locationDiscountRules[i].amount = this.amountFormatValue(this.formValues.locationDiscountRules[i].amount);
      }
    }
