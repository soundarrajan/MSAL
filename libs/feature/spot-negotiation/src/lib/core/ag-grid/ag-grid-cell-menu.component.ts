...
@Component({
  ...
  template: `
    <div *ngIf="params.type == 'cell-hover-click-menu-popup'">
      <span
        [ngStyle]="{ 'text-align': params?.align }"
        style="width: 85%;display: inline-block;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"
        >{{ params.value }}</span
      >
      <span
      ...
      <span (mouseleave)="menuTrigger.closeMenu()">
        <div class="no-border p-lr-0" *ngFor="let item of params.labels">
          <div class="hover-click-menu-popup" (click)="openPopup(item.name)">
            <div
              [class]="item.iconClass ? item.iconClass : 'no-icon-class'"
            ></div>
    ...
    <div
      *ngIf="params.type == 'hover-click-drag-menu-singleGrid'"
      class="hover-click-drag-menu"
      [ngStyle]="{
        'justify-content': params.cell_alignment ? params.cell_alignment : null
      }"
    >
      <div
        class="popup-icon info-icon hover-menu-icon"
        [matMenuTriggerFor]="hovertogglemenuSingle"
        #hovermenuTriggerSingleGrid="matMenuTrigger"
        (click)="freezeMenuOpened($event)"
        (mouseenter)="hoverPopupOpen($event)"
        (mouseleave)="hoverOutPopup()"
        (menuClosed)="freezeMenuClosed($event)"
      ></div>
      <div
    ...
    <mat-menu
      #hovertogglemenuSingle="matMenu"
      class="matmenu-blue-v2 hover-menu-white"
      xPosition="after"
      [hasBackdrop]="false"
    >
      <div
    ...
    <div
      *ngIf="params.type == 'hover-click-drag-menu-dualGrid-left'"
      class="hover-click-drag-menu"
    >
      <div
        class="popup-icon info-icon hover-menu-icon"
        [matMenuTriggerFor]="hovertogglemenuLeft"
        #hovermenuTriggerLeftGrid="matMenuTrigger"
    ...
    <div
      *ngIf="params.type == 'hover-click-drag-menu-dualGrid-right'"
      class="hover-click-drag-menu"
    >
      <div
        class="popup-icon info-icon hover-menu-icon"
        [matMenuTriggerFor]="hovermenuTriggerRight"
        #hovermenuTriggerRightGrid="matMenuTrigger"
...
export class AGGridCellMenuPopupComponent implements ICellRendererAngularComp {
  ...
  @ViewChild('hovermenuTriggerLeftGrid')
  hovermenuTriggerLeftGrid: MatMenuTrigger;
  @ViewChild('hovermenuTriggerRightGrid')
  hovermenuTriggerRightGrid: MatMenuTrigger;
  @ViewChild('hovermenuTriggerSingleGrid')
  hovermenuTriggerSingleGrid: MatMenuTrigger;
  constructor(
    ...
    this.service.getFutureSettlementTabChange().subscribe(index => {
      ...
      if (
        this.hovermenuTriggerRightGrid &&
        this.hovermenuTriggerRightGrid.menuOpen
      )
