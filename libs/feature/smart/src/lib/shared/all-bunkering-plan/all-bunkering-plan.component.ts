...
export class AllBunkeringPlanComponent implements OnInit {
  ...
  ngOnInit() {
    //Temp Variable to store the count of accordions to be displayed
    for (let i = 0; i < 20; i++) {
  ...
  public loadBunkerPlanHistory(event) {
    ...
    let fromDate = moment(this.defaultFromDate).format('yyyy-MM-DD');
    ...
    this.localService.getBunkerPlanLog(this.requestPayload).subscribe(data => {
      this.bunkerPlanLogDetail =
        data?.payload && data?.payload.length ? data.payload : {};
