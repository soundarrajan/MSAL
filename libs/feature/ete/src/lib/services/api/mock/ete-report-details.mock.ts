import {
  ...
  IEteReportDetailsProductTypeDto
...
export function getMockEteReportProductTypes(
  n: number,
  isNew: boolean = false
): IEteReportDetailsProductTypeDto[] {
...
export function getQcReportDetailsCall(id: number): IEteReportDetailsDto {
  const isNew = !id;

  const productTypeCategories = getMockEteReportProductTypes(
