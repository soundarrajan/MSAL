...
export class DatabaseManipulation extends Dexie {
  ...
  async getStatusIdByName(
    ...
    await this.db[tableName]
      .where('name')
      .startsWithAnyOfIgnoreCase(StatusName)
      .each(item => {
        entityId = item.id;
      });
    return entityId;
  ...
  public async getStatusColorFromDashboard(
    statusId: number,
    transactionTypeId: number
  ): Promise<string> {
    let color = null;
    await this.db[DatabaseManipulationEnum.DashboardLabelConfiguration]
      .where('id')
      .equals(statusId)
      .and(item => item.transactionTypeId === transactionTypeId)
      .each(item => {
        color = item.code;
      });
    return color;
