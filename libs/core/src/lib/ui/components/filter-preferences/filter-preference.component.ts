...
export class FilterPreferencesComponent implements OnDestroy {
  ...
  @Output() filterPresetsUpdate$ = new EventEmitter<FilterPreferenceViewModel[]>();
  ...
  public setPinned(id: string): void {
    this.filterPresets.find(preset => preset.id === id).isPinned = !this.filterPresets.find(preset => preset.id === id).isPinned;
  }
  ...
  public applyFilter(id: string): void {
    ...
    const filterIndex: number = this.filterPresets.findIndex(val => val.isActive);
    ...
    const currentFilterId = this.filterPresets.findIndex(preset => preset.id === id);
  ...
  public createNewFilter(): void {
    ...
    if (this.filterPresets.some(item => item.name.toLowerCase() === this.preferenceNameFormControl.value.toLowerCase())) {
    ...
    const pinnedPresets = this.filterPresets.filter(preset => preset.isPinned && !preset.isDefault && !preset.isClear);
    ...
    this.toastr.info(`Grid Preference - ${newFilter.name} Has Been Saved Successfully`, '', ToastPosition);
    ...
    this.hasActiveFilterPresets = this.filterPresets.some(item => !item.isDefault && !item.isClear);
