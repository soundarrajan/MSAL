...
import { FilterPreferenceModel, FilterPreferenceViewModel } from '../../../../services/user-settings/filter-preference.interface';
import { map, tap } from 'rxjs/operators';
import { IGridFilterChanged } from './grid-filter-changed.interface';
import { IFilterPresetsStore, IGetGridApi, PresetGroupModel } from './ag-filter-presets.models';
...
export class AgGridFilterPresetsService implements OnDestroy {
  ...
  constructor(@Inject(PREFERENCE_STORAGE) private _storage: IPreferenceStorage) {}

  // NOTE: This is used to load the current preferences and set the currently active preset to the grid
  public loadPresets(presetGroupKey: string): Observable<FilterPreferenceViewModel[]> {
    ...
    return this._storage.get<FilterPreferenceViewModel[]>(presetGroupKey).pipe(
      map(response => (response || []).map(item => new FilterPreferenceViewModel(item))),
      tap(preferences => {
        ...
        this.filterPresets[presetGroupKey] = new PresetGroupModel({ items: _.cloneDeep(defaultFilterItems) });
        this.filterPresets[presetGroupKey].items = [...this.filterPresets[presetGroupKey].items, ...preferences];
        if (!this.filterPresets[presetGroupKey].items.some(item => item.isActive)) {
          this.filterPresets[presetGroupKey].items.find(item => item.isClear).isActive = true;
        }
  ...
  public addFilterPresetsToExistingGroup(presetGroupKey: string, presets: FilterPreferenceViewModel[], unshift: boolean = false): void {
    if (!this.filterPresets || !this.filterPresets[presetGroupKey]) {
      return;
    }

    if (_.difference(presets, this.filterPresets[presetGroupKey].items).length !== 0) {
      this.filterPresets[presetGroupKey].items = unshift ? [...presets, ...this.filterPresets[presetGroupKey].items] : [...this.filterPresets[presetGroupKey].items, ...presets];
    }
  }
  ...
  public saveFilterPreset(presetGroupKey: string): void {
    ...
    const updatedFilter: FilterPreferenceViewModel = this.filterPresets[presetGroupKey].items.find(filter => filter.isActive);
    ...
    _.keys(this.gridApis[presetGroupKey]).forEach(key => {
      updatedFilter.filterModels[key] = this.gridApis[presetGroupKey][key]().getFilterModel();
    });

    // NOTE: Setting the updated preset status as of now it doesn't have any unsaved changes
    updatedFilter.hasChanges = false;

    this._storage.set(presetGroupKey, this.filterPresets[presetGroupKey].items.filter(item => !item.isDefault && !item.isClear).map(item => new FilterPreferenceModel(item))).subscribe();
  }

  // NOTE: When a change occurs on the presets this is called to update the service presets store value
  public setGridFilterPresets(presetGroupKey: string, items: FilterPreferenceViewModel[]): void {
  ...
  public setFilterModel(presetGroupKey: string, presets: FilterPreferenceViewModel[]): void {
  ...
  public setGridFilterModel(presetGroupKey: string): void {
    ...
    const activeFilter = this.filterPresets[presetGroupKey].items[this.filterPresets[presetGroupKey].items.findIndex(val => val.isActive)];

    if (activeFilter) {
      _.keys(this.gridApis[presetGroupKey]).forEach(key => {
        if (this.gridApis[presetGroupKey][key]) {
          this.gridApis[presetGroupKey][key]().setFilterModel(activeFilter.filterModels[key]);
        }
      });
    }
  }
  ...
  public setAllInactive(presetGroupKey: string): void {
    this.filterPresets[presetGroupKey].items.forEach(item => (item.isActive = false));
  }

  public setActivePreset(presetGroupKey: string, presetId: string): void {
    const activePreset = this.filterPresets[presetGroupKey].items.find(preset => preset.id === presetId);
