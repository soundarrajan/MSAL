...
<#
.SYNOPSIS
Imports resource strings for use with Get-LocString.

.DESCRIPTION
Imports resource strings for use with Get-LocString. The imported strings are stored in an internal resource string dictionary. Optionally, if a separate resource file for the current culture exists, then the localized strings from that file then imported (overlaid) into the same internal resource string dictionary.
