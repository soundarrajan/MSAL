angular.module('shiptech.pages').controller('NewOrderController', [ 'API', '$scope', '$element', '$listsCache', '$attrs', '$timeout','$http', '$state', '$filter', '$stateParams', '$templateCache', '$tenantSettings', '$uibModal', 'STATE', 'SCREEN_LAYOUTS', 'LOOKUP_TYPE', 'LOOKUP_MAP', 'IDS', 'ORDER_COMMANDS', 'VALIDATION_MESSAGES', 'SCREEN_ACTIONS', 'COST_TYPE_IDS', 'COMPONENT_TYPE_IDS', 'EMAIL_TRANSACTION', 'uiApiModel', 'listsModel', 'orderModel', 'lookupModel', 'screenActionsModel', 'tenantService', 'newRequestModel', '$uibModal', 'Factory_Master', 'Factory_Admin', '$rootScope', '$compile', 'statusColors', '$window', '$location',
    function(API, $scope, $element, $listsCache, $attrs, $timeout, $http, $state, $filter, $stateParams, $templateCache, $tenantSettings, uibModal, STATE, SCREEN_LAYOUTS, LOOKUP_TYPE, LOOKUP_MAP, IDS, ORDER_COMMANDS, VALIDATION_MESSAGES, SCREEN_ACTIONS, COST_TYPE_IDS, COMPONENT_TYPE_IDS, EMAIL_TRANSACTION, uiApiModel, listsModel, orderModel, lookupModel, screenActionsModel, tenantService, newRequestModel, $uibModal, Factory_Master, Factory_Admin, $rootScope, $compile, statusColors, $window, $location) {
        ...
        function loadData(data) {
            ...
            ctrl.data.products = $filter('orderBy')(ctrl.data.products, 'productType.orderBy');
        ...
        function calculateProductAmount(product) {
            ...
            return (Number(convertDecimalSeparatorStringToNumber(confirmedQuantityOrMaxQuantity)) || 0) * (Number(convertDecimalSeparatorStringToNumber(product.confirmedQtyPrice)) || 0) * (Number(convertDecimalSeparatorStringToNumber(product.price)) || 0);
        }
        ...
        ctrl.confirmQuantityChanged = function(source, data) {
            ...
            if (ctrl.data && ctrl.data.status && ctrl.data.status.name == 'Confirmed') {
                if (data.minQuantity == null || data.minQuantity == undefined) {
                    data.minQuantity = prodQuantity;
                }
                if (data.maxQuantity == null || data.maxQuantity == undefined) {
                    data.maxQuantity = prodQuantity;
                }
            }
        };
        ...
        function productUomChg(product, isPriceUom) {
            ...
            if (!isPriceUom) {
                if (product.quantityUom.id == product.priceUom.id) {
                    ...
                    product.amount = Number(convertDecimalSeparatorStringToNumber(product.confirmedQtyPrice)) * Number(convertDecimalSeparatorStringToNumber(confirmedQuantityOrMaxQuantity)) * Number(convertDecimalSeparatorStringToNumber(product.price));
                } else {
                	if (!product.contractProductId) {
        ...
        ctrl.openConversionFactorModel = function(isUnitPriConvFac, convFactData) {
            ...
            let metricTonUom = _.find(ctrl.listsCache.Uom, function(object) {
                if(object?.name.toLowerCase() == 'mt')
                    return object;
            });
            let cubicMeterUom = _.find(ctrl.listsCache.Uom, function(object) {
                if(object?.name.toLowerCase() == 'cbm')
                    return object;
            });
            let metricMillionBritishThermalUnitUom = _.find(ctrl.listsCache.Uom, function(object) {
                if(object?.name.toLowerCase() == 'mmbtu')
                    return object;
            });
            let mWHUom = _.find(ctrl.listsCache.Uom, function(object) {
                if(object?.name.toLowerCase() == 'mwh')
                    return object;
            });         
            var request_payload = [];
            ...
            lookupModel.getUOMConversionFactor(request_payload).then((server_data) => {
                server_data.payload.forEach((cv) => { 
                    if(!isUnitPriConvFac && cv.id == ctrl.conversionFactorData.quantityUom.id)
                    {
                    ...
                    else if(isUnitPriConvFac && cv.id == ctrl.conversionFactorData.priceUom.id)
                    {
