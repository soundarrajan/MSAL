...
(function() {
    ...
    let multiselect = angular.module('shiptech.pages');
    ...
    angular.module('shiptech.pages').directive('multiselect', [ '$filter', '$document', '$log', function($filter, $document, $log) {
        return {
            restrict: 'AE',
            scope: {
                options: '=',
            ...
            require: 'ngModel',
            template: String('<div class="btn-group" style="width: 100%">\n' +
		    ...
		    '        ng-style="{display: open ? \'block\' : \'none\'}" style="width: 100%; overflow-x: auto; position: absolute; max-width: 175px;">\n' +
		    '\n' +
            ...
            link: function($scope, $element, $attrs, $ngModelCtrl) {
                ...
                let closeHandler = function(event) {
                    if (!$element[0].contains(event.target)) {
                        // $scope.open = false;
                        $($element[0]).find('.dropdown-menu').css('display', 'none');
                        // $scope.$apply();
                    } else {
                        $($element[0]).find('.dropdown-menu').css('display', 'block');
                    }
                };

                $document.on('click', closeHandler);

                let updateSelectionLists = function() {
                ...
                let watcher = $scope.$watch('selectedOptions', () => {
                ...
                $scope.getButtonText = function() {
                    ...
                    if ($scope.selectedOptions && $scope.selectedOptions.length > 1) {
                        let totalSelected = angular.isDefined($scope.selectedOptions) ? $scope.selectedOptions.length : 0;
                        if (totalSelected === 0) {
                ...
                $scope.toggleItem = function(item) {
                    ...
                    let selectedIndex = $scope.selectedOptions.indexOf(item);
                    let currentlySelected = selectedIndex !== -1;
                    if (currentlySelected) {
                        ...
                        let unselectedIndex = $scope.unselectedOptions.indexOf(item);
                ...
                $scope.isSelected = function(item) {
                    ...
                    let itemId = $scope.getId(item);
                    for (let i = 0; i < $scope.selectedOptions.length; i++) {
                        let selectedElement = $scope.selectedOptions[i];
                ...
                $scope.search = function() {
                    ...
                    return function(item) {
                        ...
                        let displayName = $scope.getDisplay(item);
                        if (displayName) {
                            let result = displayName.toLowerCase().indexOf($scope.searchFilter.toLowerCase()) > -1;
                            if (result) {
                                counter++;
                            }
                            return result;
