        ...
        function($scope, $element, $attrs, $filter, $timeout, lookupModel, uiApiModel, newRequestModel, groupOfRequestsModel, tenantService,
                ...
                groupOfRequestsModel.getOfferDetails(changes.args.currentValue.offer.id)
                    .then((data) => {
                        ...
                        for (let i = 0; i < ctrl.location.products.length; i++) {
                            let prod = ctrl.location.products[i];
                            for (var k = 0; k < prod.sellers[0].offers[0].additionalCosts.length; k++) {
                                let ac = prod.sellers[0].offers[0].additionalCosts[k];
                                addPriceUomChg(ac, ctrl.location);
                            }
                        }

                        // Get the counterparty contacts to use in the Quoted by select control.
                        lookupModel.getCounterpartyContacts(ctrl.requestProduct.sellers[0].sellerCounterparty.id).then((data) => {
                ...
                if (ctrl.location.products) {
                    for(let i = 0; i < ctrl.location.products.length; i++) {
                        ...
                        if (product.sellers[0].offers[0].additionalCosts) {
                            for(let j = 0; j < product.sellers[0].offers[0].additionalCosts.length; j++) {
            ...
            */
            ...
            function calculateProductAmount(product) {
                return Number(product.confirmedQtyPrice) * Number(product.maxQuantity) * Number(product.sellers[0].offers[0].price) || 0;
            }
            ...
            function sumProductMaxQuantities(products) {
                ...
                for(let i = 0; i < products.length; i++) {
                    result = result + products[i].maxQuantity;
                }
            ...
            */
            ctrl.sumProductAmounts = function(products) {
                ...
                for(let i = 0; i < products.length; i++) {
                    result = result + products[i].totalAmount;
                }
                return result;
            };
            ...
            */
            function addFirstAdditionalCost() {
                for(let i = 0; i < ctrl.data.locations.length; i++) {
            ...
            */
            function setProductPricingType() {
                for(let i = 0; i < ctrl.data.locations.length; i++) {
                    for(let j = 0; j < ctrl.data.locations[i].products.length; j++) {
            ...
            */
            ...
            function productUomChg(product) {
                if (product.uom.id == product.sellers[0].offers[0].priceQuantityUom.id) {
                    ...
                    product.totalAmount = Number(product.confirmedQtyPrice) * Number(product.maxQuantity) * Number(product.sellers[0].offers[0].price);
                } else {
                    lookupModel.getConvertedUOM(product.product.id, 1, product.uom.id, product.sellers[0].offers[0].priceQuantityUom.id).then((server_data) => {
