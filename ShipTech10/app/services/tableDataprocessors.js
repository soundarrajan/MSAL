angular.module('shiptech').service('dataProcessors', [ '$filtersData', '$state', '$rootScope', function($filtersData, $state, $rootScope) {
        ...
        $.each(colmodel, (key, obj) => {
            ...
            if(obj.label.toLowerCase().indexOf('company') >= 0) {
                if (typeof tenantSettings.companyDisplayName != 'undefined') {
                    if(obj.label.indexOf('Company') >= 0) {
                        colmodel[key].label = obj.label.replace('Company', tenantSettings.companyDisplayName.name);
                    }
                    if(obj.label.indexOf('company') >= 0) {
                        colmodel[key].label = obj.label.replace('company', tenantSettings.companyDisplayName.name.toLowerCase());
                    }
                }
            }
            ...
            ***********/
            ...
            if (currentList == 'contract-planning/') {
                currentList = 'contract-planning';
            }
            if (currentList.indexOf('contract-planning') >= 0) {
                currentList = 'contract-planning';
            }
    ...
    function normalizeTableData(tableData) {
        ...
        for (let i = 0; i < tableData.length; i++) {
            dataRow = tableData[i];
            for (let j = 0; j < dataRow.voyageDetail.length; j++) {
