...
namespace Shiptech.Negotiation.Api.Controllers
{
    ...
    public class GroupsController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult<CreateGroupResponse>> CreateGroup(CreateGroupRequest requestDto)
        {
            var tenantInfo = _httpContextAccessor.HttpContext.GetTenantInfoFromContext();
            requestDto.TenantId = tenantInfo.TenantId;
        ...
        public async Task<ActionResult<AddCounterpartiesResponse>> AddCounterparties([FromBody] List<RequestLocationSellerDto> requestDto)
        {
            var response = await _mediator.Send(new AddCounterpartiesRequest { counterparties = requestDto });
            return response;
        }
    }
}
