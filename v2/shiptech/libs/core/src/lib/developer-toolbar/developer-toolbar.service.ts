...
export class DeveloperToolbarService implements OnDestroy {
   ...
   * @param fromSettings
   */
  public patchCallSettings(
    toSettings: IMethodApiCallSettings[],
    fromSettings: IMethodApiCallSettings[]
  ): void {
    toSettings.forEach(toMethod => {
      ...
      const toNextResponse = toMethod.settings.cannedResponses.find(
        c =>
          c.name ===
          (fromMethod.settings.nextResponse || <ICannedResponse>{}).name
      );
      ...
      toMethod.settings.nextResponse =
        toNextResponse || toMethod.settings.nextResponse;
    });
  }

  public getApiSettings(apiServiceId: string): IApiServiceSettings {
    const storedSettingsJson = localStorage.getItem(
      `${DEV_SETTINGS_STORAGE_PREFIX}_${apiServiceId}`
    );
  ...
  public saveApiSettings(
    apiServiceId: string,
    settings: IApiServiceSettings
  ): void {
  ...
  public shouldKeepSettings(): boolean {
    return (
      Object.keys(localStorage).filter(key =>
        key.startsWith(DEV_SETTINGS_STORAGE_PREFIX)
      ).length > 0
    );
  }
  ...
  public getApiCallMetadata(source: any): IMethodApiCallSettings[] {
    ...
    return [
      ...
      ...Object.keys(Object.getPrototypeOf(source))
    ]
      .map(
        key =>
          <IMethodApiCallSettings>Reflect.getMetadata(API_CALL_KEY, source, key)
      )
      .filter(m => m);
  }
}
