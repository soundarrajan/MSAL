...
class ShiptechMasterUom {
    ...
    async NewUom(testCase, commonTestData) {
        ...
        if (isFound) {
            this.tools.log("Uom already exists " + testCase.Name);
            return;
        }
        ...
        if (!isFound)
            throw new Error("Uom not found in list: \"" + testCase.Name + "\"");
    ...
    async SearchInList(testCase, commonTestData) {
        testCase.url = "masters/uom";
        testCase.pageTitle = "Uom List";
        ...
        var labelTitle = await this.tools.getText("a[href='#/masters/uom']");
    ...
    async AddItem(testCase, commonTestData) {        
        ...
        await this.tools.getPage("Uom", false, true);
        await this.tools.setText("#UOMName", testCase.Name);        
        await this.tools.selectFirstOptionBySelector("#UomTypeUomType");
        await this.tools.selectBySelector("#UomTypeUomType", testCase.UOMType);
        await this.tools.setText("#ConversiontoBaseUOM", testCase.ConversionToBaseUom);
        
        await this.tools.click("#header_action_save");
        await this.tools.waitForLoader();

        var name = await this.tools.getTextValue("#UOMName");
        if (name != testCase.Name)
            throw new Error("Cannot save UOM " + testCase.Name + " current value: " + name);

        this.tools.log("UOM saved " + testCase.Name);
    }
}



module.exports = ShiptechMasterUom;