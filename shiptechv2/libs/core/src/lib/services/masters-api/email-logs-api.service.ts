...
export class EmailLogsApi implements IEmailLogsApiService {
  ...
  getEmailLogs(request: IEmailLogsRequest): Observable<IEmailLogsResponse> {
    return this.http.post<IEmailLogsResponse>(
      `${this._apiUrl}/${EmailLogsApiPaths.getEmailLogs()}`,
      { payload: { ...request } }
    );
  }
}

export const EMAIL_LOGS_API_SERVICE = new InjectionToken<IEmailLogsApiService>(
