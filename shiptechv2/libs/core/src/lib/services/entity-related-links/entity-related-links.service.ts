...
export class EntityRelatedLinksService extends BaseStoreService {
  constructor(
    ...
    @Inject(ENTITY_RELATED_LINKS_API) private api: IEntityRelatedLinksApi,
    store: Store,
  ...
  getRelatedLinksForEntity(
    ...
    id: any
  ): Observable<IEntityRelatedLink[]> {
    return this.api
      .getRelatedLinksForEntity(
        EntityToEntityIdFieldMap[entityType],
        Number(id)
      )
      .pipe(
        map(response =>
          AllEntityTypes.map(t =>
            this.tryMapToEntityRelatedLink(t, response)
          ).filter(e => e)
        )
      );
  }
  ...
  private tryMapToEntityRelatedLink(
    type: EntityType,
    dto: EntityRelatedLinksResponse
  ): IEntityRelatedLink | undefined {
