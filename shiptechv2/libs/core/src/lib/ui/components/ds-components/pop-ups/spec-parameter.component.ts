...
@Component({
    selector: 'spec-parameter-dialog',
    template:
        `
      ...
      <div>
        <mat-dialog-content>
          <ag-grid-angular domLayout='autoHeight' style="width: 100%;height: 100%;" [gridOptions]="spec_dialog_gridOptions" class="ag-popupgrid-v2 ag-theme-material angular-v9">
          </ag-grid-angular>
        </mat-dialog-content>
      </div>
      <div>
...
export class SpecParameterDialog {

    public spec_dialog_gridOptions: GridOptions;

    ngOnInit() {
    }
    constructor(
        public dialogRef: MatDialogRef<SpecParameterDialog>,
        @Inject(MAT_DIALOG_DATA) public data: any) {
        this.spec_dialog_gridOptions = <GridOptions>{
            defaultColDef: {
                filter: false,
                sortable: false,
                resizable: true
            ...
            rowHeight: 28,
            onGridReady: params => {
                this.spec_dialog_gridOptions.api = params.api;
                this.spec_dialog_gridOptions.columnApi = params.columnApi;
                this.spec_dialog_gridOptions.api.sizeColumnsToFit();
                this.spec_dialog_gridOptions.api.setRowData(this.rowData);
    ...
    addCustomHeaderEventListener(params) {
        ...
        addButtonElement[0].addEventListener('click', (event) => {
            params.api.applyTransaction({
                add: [{
                    specParam: 'TAN', minmax: 'Min', minimum: '0', method: 'Example', maximum: '0', uom: "MT", name: 'Barging', bindingParam: "",
                }]
            });
        });
    ...
    public columnDefs = [
        ...
        { headerName: 'Testing Method', editable: true, singleClickEdit: true, headerTooltip: 'Testing Method', field: 'method', cellClass: ['editable-cell'] },
        {
            headerName: 'Binding Parameter', suppressMenu: true, headerTooltip: 'Binding Parameter', field: 'bindingParam', cellClass: ['aggrid-text-align-c'],
            cellRendererFramework: AGGridCellActionsComponent, cellRendererParams: { type: 'radio-button-selection' }
        },
    ];

    private rowData = [
        {
            specParam: 'TAN', minmax: 'Min', minimum: 0, method: 'Example', maximum: 5, uom: "MT", bindingParam: false,
        },
        {
            specParam: 'TAN', minmax: 'Min', minimum: 0, method: 'Example', maximum: 5, uom: "MT", bindingParam: false,
        },
        {
            specParam: 'TAN', minmax: 'Min', minimum: 0, method: 'Example', maximum: 5, uom: "MT", bindingParam: true,
        },
        {
            specParam: 'TAN', minmax: 'Min', minimum: 0, method: 'Example', maximum: 5, uom: "MT", bindingParam: false,
        }
