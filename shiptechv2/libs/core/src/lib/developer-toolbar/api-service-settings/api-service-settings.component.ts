...
export class ApiServiceSettingsComponent implements OnDestroy {
  @Input() set reset$(value: Observable<any>) {
    ...
    this._resetSubscription = this._reset$.subscribe(() => {
      const settings = <IApiServiceSettings>(
        JSON.parse(this.originalSettingsJson)
      );
      this.applySettings(settings);
      this.devService.saveApiSettings(
        this.apiService.id,
        this.getCurrentSettingsForSaving()
      );
    });
  }
  ...
  set apiService(value: IApiService) {
    ...
    this.originalSettingsJson = JSON.stringify(
      this.getCurrentSettingsForSaving()
    );
  ...
  changeForwardCallToForAll(checkboxChange: MatCheckboxChange): void {
    this.methods.forEach(
      method => (method.settings.forwardToReal = checkboxChange.checked)
    );
  ...
  updateForwardAllToReal(): void {
    const checkState = (test: boolean) =>
      this.methods.every(method => method.settings.forwardToReal === test);
  ...
  changeThrowErrorToForAll(checkboxChange: MatCheckboxChange): void {
    this.methods.forEach(
      method => (method.settings.throwError = checkboxChange.checked)
    );
  ...
  updateThrowErrorForAll(): void {
    const checkState = (test: boolean) =>
      this.methods.every(method => method.settings.throwError === test);
   ...
   */
  updateDelayAll(): void {
    const allSameDelay = this.methods.every(
      (method, i, arr) => method.settings.delay === arr[0].settings.delay
    );

    if (this.methods.length) {
      this.selectedDelayAll =
        this.methods.length > 0 && allSameDelay
          ? this.methods[0].settings.delay
          : undefined;
    }
  ...
  updateOverrideAll(): void {
    const allSameValue = this.methods.every(
      (method, i, arr) => method.settings.apiUrl === arr[0].settings.apiUrl
    );

    this.overrideAllUrls =
      this.methods.length > 0 && allSameValue
        ? this.methods[0].settings.apiUrl
        : undefined;
    this.devService.saveApiSettings(
      this.apiService.id,
      this.getCurrentSettingsForSaving()
  ...
  private loadApiCallSettings(): void {
    ...
    this.methods.forEach(
      method =>
        (method.settings.apiUrl = getApiCallUrlValue(
          this.apiService.instance,
          true
        ))
    );
  ...
  private applySettings(settings: IApiServiceSettings): void {
    if (
      ...
      settings.methodSettings.length === 0
    ) {
      return;
    }
    ...
    const devSelectedRackApiCall = this.methods.find(
      s => s.name === settings.selectedMethodName
    );
  ...
  private getCurrentSettingsForSaving(): IApiServiceSettings {
    return {
      ...
      selectedMethodName: (this.selectedMethod || <IMethodApiCallSettings>{})
        .name,
