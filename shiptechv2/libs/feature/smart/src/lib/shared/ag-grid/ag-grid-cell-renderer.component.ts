...
@Component({
  ...
  template: `
    <div
      *ngIf="this?.params?.colDef?.filter !== 'date'"
      [ngClass]="params.cellClass"
      [ngStyle]="params.cellStyle"
    ...
    <div
      *ngIf="this?.params?.colDef?.filter == 'date'"
      [ngClass]="params.cellClass"
      matTooltip="{{ params.value | utcDatePipe | date: 'dd/MM/yyyy' }}"
      style="margin:0px"
    >
      {{ params.value | utcDatePipe | date: 'dd/MM/yyyy' }}
    </div>
  `
})
