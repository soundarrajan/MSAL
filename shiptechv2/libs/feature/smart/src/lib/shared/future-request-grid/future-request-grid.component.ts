...
export class FutureRequestGridComponent implements OnInit {
    ...
    return new Promise(resolve => {
      ...
      filterModelArr.forEach((filterValue, index) => {
        if (filterModel[filterValue]?.operator) {
          let conditionArr = Object.keys(filterModel[filterValue]).filter(
            condKey => condKey.indexOf('condition') > -1
          );
          conditionArr.forEach(async (key, idx) => {
            filterModel[filterValue][key]['operator'] =
              filterModel[filterValue]?.operator;
            conditionFilterArr[filterValue + ('~~~' + idx + 1)] =
              filterModel[filterValue][key];
          });
        } else {
  ...
  generateFilterModel(params) {
    ...
    return new Promise(async resolve => {
      ...
      const vesselRef = this.store.selectSnapshot(
      ...
      if (
        ...
        this.columnFilter = this.columnFilter.filter(
          EtaItem => EtaItem.columnValue == 'Eta'
        );
        ...
        filterModelArr.forEach(async (filterKey, index) => {
          ...
          columnFormat.columnValue = await this.mapColumnValue(
            filterKey.split('~~~')[0]
          );
          columnFormat.ColumnType =
            filterType.charAt(0).toUpperCase() + filterType.substring(1);
          columnFormat.ConditionValue = await this.mapConditionType(
            filterModel[filterKey]?.type
          );
          columnFormat.Values = await this.mapFilterValue(
            filterType,
            filterModel[filterKey]
          );
          let isColumnMultiFilter = this.columnFilter.filter(
            filterItem => filterItem.columnValue == columnFormat.columnValue
          );
          if (isColumnMultiFilter.length) {
            let filterLastIndex = this.columnFilter
              .map(filterItem => filterItem.columnValue)
              .lastIndexOf(columnFormat.columnValue);
            let opertorMode = filterModel[filterKey]?.operator;
            if (filterLastIndex != -1) {
              this.columnFilter[filterLastIndex].FilterOperator =
                opertorMode == 'AND' ? 1 : opertorMode == 'OR' ? 2 : 0;
            } else {
              columnFormat.FilterOperator =
                opertorMode == 'AND' ? 1 : opertorMode == 'OR' ? 2 : 0;
            }
          }
          ...
          if (filterModelArr.length == index + 1) {
            ...
            if (vesselRef && vesselRef.vesselId && this.filterByVessel) {
              requestPayload.Payload.PageFilters.Filters.push(
                columnFilterByVessel
              );
            }
