        ...
        return next.handle(req).pipe(
            map(resp => {
               if (resp instanceof HttpResponse && !(<any>window).qcActions) {
                ...
                if (resp.url.indexOf('admin/audit/get') != -1 || resp.url.indexOf('masters/emaillogs/list') != -1 || resp.url.indexOf('masters/documentupload/list') != -1 || resp.url.indexOf('quantityControlReport/details') != -1) {
                    this.loaderService.isLoading.next(true);
                }
                return  resp;
              } else {
