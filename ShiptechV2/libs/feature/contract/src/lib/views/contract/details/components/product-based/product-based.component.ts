...
export class ProductBased extends DeliveryAutocompleteComponent
  implements OnInit{
  ...
  @Input('model') set _setFormValues(formValues) { 
    ...
    if (this.formValues.productDiscountRules && this.formValues.productDiscountRules.length) {
      this.formatValues();
    }
  }
  ...
  formatValues() {
    for (let i = 0; i < this.formValues.productDiscountRules.length; i++) {
      if (this.formValues.productDiscountRules[i].amount) {
        this.formValues.productDiscountRules[i].amount = this.amountFormatValue(this.formValues.productDiscountRules[i].amount);
      }
    }
