...
import { ModuleError } from '../../../../../spot-negotiation/src/lib/views/main/details/components/negotiation-documents/error-handling/module-error';
  ...
  template: `
    ...
    <div *ngIf="params.type === 'dashed-border-notes'" class="dashed-border-note">
      <div class="dashed-border-notes">
        <input matInput [(ngModel)]="docNotes" matTooltip="{{ docNotes }}" autocomplete="off" (blur)="updateNotes()" />
      </div>
    </div>
  `
})
export class AGGridCellActionsDocumentsComponent implements ICellRendererAngularComp {
  docNotes: string;
  public params: any;
  public popupOpen: boolean;
  constructor(private _FileSaverService: FileSaverService, private toastr: ToastrService, private spinner: NgxSpinnerService, public router: Router, public dialog: MatDialog, private CommonApiService: CommonApiService, private confirmationService: ConfirmationService, private appErrorHandler: AppErrorHandler) {}
  ...
  deleteDocument(): void {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.CommonApiService.deleteDocument(request).subscribe(
          () => {
  ...
  downloadDocument(): void {
    ...
    this.spinner.show();
    this.CommonApiService.downloadDocument(request).subscribe(
      response => {
        this.spinner.hide();
  ...
  updateNotes(): void {
    if (this.params.node.data.notes !== this.docNotes) {
      ...
      this.spinner.show();
      this.CommonApiService.updateNotes(request).subscribe(
        response => {
          this.spinner.hide();
