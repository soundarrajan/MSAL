...
export class VesselDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.localService.vesselPopUpDetails.subscribe(
      value => (this.vesselData = value)
    );
    ...
    this.store.dispatch(
      new saveVesselDataAction({ vesselRef: this.vesselData })
    );
  }
    ...
    this.localService.checkVesselHasNewPlan(this.vesselId).subscribe(data => {
      ...
      if (data.planCount > 0) {
        ...
        const confirmMessage =
          this.selectedUserRole?.name == 'Vessel'
          ...
          if (result) {
            ...
            this.store.dispatch(
              new saveVesselDataAction({
                userRole: this.selectedUserRole?.name
              })
            );
            ...
            setTimeout(() => {
              _this.selectedRole = _this.bunkerUserRole.find(
                role => role.name == _this.previousUserRole?.name
              );
              _this.selectedUserRole = _this.selectedRole;
              // store user role for shared ref
              this.store.dispatch(
                new saveVesselDataAction({
                  userRole: _this.selectedUserRole?.name
                })
              );
  ...
  checkVesselHasNewPlan() {
    ...
    this.localService.checkVesselHasNewPlan(this.vesselId).subscribe(data => {
      data = data.payload?.length ? data.payload[0] : data.payload;
      if (data.planCount > 0) this.vesselWarningConfirmation();
    });
  }
  ...
  vesselWarningConfirmation() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      setTimeout(() => {
        ...
        // store user role for shared ref
        this.store.dispatch(
          new saveVesselDataAction({ userRole: _this.selectedUserRole?.name })
        );
        let titleEle = document.getElementsByClassName(
          'page-title'
        )[0] as HTMLElement;
