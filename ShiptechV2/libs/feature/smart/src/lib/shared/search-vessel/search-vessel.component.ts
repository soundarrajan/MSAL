...
export class SearchVesselComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    if (this.vesselRef?.code) {
      this.searchVesselControl.setValue(
        this.vesselRef && this.vesselRef.code ? this.vesselRef.code : ''
      );
      this.selectedValue =
        this.vesselRef && this.vesselRef.code ? this.vesselRef.code : '';
    } else {
      ...
      this.vesselRef = this.vesselList.find(
        element => element.code == selectedVesselCode
      );
      this.searchVesselControl.setValue(
        this.vesselRef && this.vesselRef.code ? this.vesselRef.code : ''
      );
    ...
    if (filterValue == '') return;
      ...
      return this.filterList.filter(option => {
        if (option?.displayName && option?.code) {
          return (
            (option?.displayName).toLowerCase().indexOf(filterValue) > -1 ||
  ...
  onVesselSelected(trigger: MatAutocompleteTrigger) {
    ...
    let vessel = this.vesselList.filter(element => {
      if (element.code) {
        return (
          element.code == this.searchVesselControl.value ||
          element.code.toLowerCase() ==
            this.searchVesselControl.value.toLowerCase() ||
