...
export class NotificationsComponent implements OnInit {
  @ViewChildren(MatExpansionPanel) matExpansionPanelQueryList: QueryList<
    MatExpansionPanel
  >;
  @ViewChild('createPanelOpen', { static: false })
  createPanelOpen: MatExpansionPanel;
  ...
  checkEmptyOnAddMore() {
    this.addTriggerRules.forEach((item, index) => {
      ...
      if (
        optionLength == 0 ||
        optionLength != this.alertAddTags[index].addMoreTags.length ||
  ...
  addingMoreRules(type, index, tindex) {
    if (type == 'add') {
      ...
      if (!this.errorsAlerts[tindex].nonEqualConditions)
        this.addMoreRules[tindex].tags.push(
          this.addMoreRules[tindex].tags.length
        );
    } else {
  ...
  createAlertFormSubmit(event, a_id) {
    ...
    if (this.createAlert.title && !titleError && !msgError) {
      //&& this.createAlert.ruletype && !msgError && !conditionError && !optionError && !titleError){
      if (a_id > -1) {
      ...
      this.addTriggerRules.forEach((item, index) => {
        ...
        this.addMoreRules[index].tags.forEach((aitem, vals) => {
          ...
          addMoreValues.push({
            ...
            columnType:
              this.alertAddTags[index].addMoreTags[vals] == 'Bunker plan delay'
                ? 1
                : this.alertAddTags[index].addMoreTags[vals] ==
                  'Operating Region'
                ? 2
                : 3,
            conditionValue:
              this.alertAddTags[index].conditionTags[vals] == 'less than'
                ? '<'
                : this.alertAddTags[index].conditionTags[vals] == 'greater than'
                ? '>'
                : 'in',
  ...
  loadAlertParameters() {
    let requestPayload = '';
    this.alertservice
      .getAlertParamters(requestPayload)
      .subscribe((res: any) => {
        let typeId = [];
        ...
        for (let key in this.allAlertTags) {
          ...
          if (!typeId.includes(this.allAlertTags[key].alertMasterId)) {
            ...
            this.alertsType.push({
              key: this.allAlertTags[key].alertMasterId,
              value: this.allAlertTags[key].alertType
            });
          }
        }
      });
