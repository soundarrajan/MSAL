...
export class MasterAutocompleteComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    if (!this.autoComplete) {
      throw Error(`${this.constructor.name} requires a content child of type ${AutoComplete.name}`);
    }
  }
  ...
  selectorSelectionChange(selection: IDisplayLookupDto | IDisplayLookupDto[]): void {
    if (selection === null || selection === undefined)
      this.autoComplete?.selectItem(undefined);
    else
      this.autoComplete?.selectItem(selection, false);
  }
