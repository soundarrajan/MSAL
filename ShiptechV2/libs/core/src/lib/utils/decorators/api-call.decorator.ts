...
export function getApiCallUrlValue(
  target: any,
  targetIsMock: boolean = true
): any {
...
function logMethodCall(
  ...
  console.groupCollapsed(
    `%c${isMocked ? 'Mock' : 'Real'} Request: ${methodName}`,
    'color: blue;'
  );

  Array.from(methodArgs).forEach((arg: any, index: number) =>
    console.log(
      `%c${argNames[index]}: %o`,
...
function logMethodReturn(
  ...
  console.groupCollapsed(
    `%c${isMocked ? 'Mock' : 'Real'} Response: ${methodName}`,
    'color: green;'
  );
...
export function ApiCall(
  ...
  return (
    ...
    const methodSettings: IMethodApiCallSettings = {
      name: methodName,
      settings
    };
    ...
    descriptor.value = function(...args: any[]): any {
      ...
      const callDelay =
        settings.delay === RANDOM_DELAY
          ? getRandomInt(settings.delayMin, settings.delayMax)
          : settings.delay;
      const callToMethod = settings.forwardToReal
        ? getForwardMethod(this, methodName) || originalMethod
        : originalMethod;
      ...
      if (!isObservable(originalResponse)) {
        console.warn(
          `Response of API call: ${methodName} was not an Observable. Response will not be delayed. Actual response: %o`,
          originalResponse
        );
      ...
      const response$ =
        settings.nextResponse === defaultCannnedResponse
          ? originalResponse
          : settings.nextResponse.response;
      const catchErrors$ = catchError(err => {
        logMethodReturnError(methodName, err, isMocked);
        return throwError(err);
      });
      const logResponse$ = tap(result =>
        logMethodReturn(methodName, result, isMocked)
      );

      if (callDelay === 0) {
        return response$.pipe(catchErrors$, logResponse$);
      } else {
        return timer(callDelay).pipe(
          first(),
          concatMap(() => response$),
