...
const dotnetFormatMapping = {
  ...
  dd: 'DD',
  DDD:
    'ddd' /* This is a hack around legacy shiptech, DDD does not exist in dotnet */,
  y: 'YYYY',
  yy: 'YY',
  yyy: 'YYYY',
  yyyy: 'YYYY',
...
export class DotNetMomentFormatAdapter {
  ...
  toDotNetFormat(formatString: string): string {
    if (!momentDateFormats[formatString]) {
      momentDateFormats[formatString] = this.translateFormat(
        formatString,
        momentFormatMapping
      );
    }
  ...
  private translateFormat(
    ...
    for (; i < len; i++) {
      ...
      if (lastChar === null || lastChar !== currentChar) {
        // change detected
        resultString = this.appendMappedString(
          formatString,
          mapping,
          startIndex,
          i,
    ...
    return this.appendMappedString(
      formatString,
      mapping,
      startIndex,
      i,
