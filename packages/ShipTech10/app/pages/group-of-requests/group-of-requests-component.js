angular.module("shiptech.pages").controller("GroupOfRequestsController", [
    ...
    function ($scope, $rootScope, $element, $compile, $attrs, $timeout, $interval, $uibModal, $templateCache, $listsCache, $filter, $state, $stateParams, $http, Factory_Master, STATE, API, SCREEN_ACTIONS, screenActionsModel, listsModel, uiApiModel, lookupModel, groupOfRequestsModel, newRequestModel, tenantService, notificationsModel, LOOKUP_TYPE, LOOKUP_MAP, SCREEN_LAYOUTS, SELLER_SORT_ORDER, EMAIL_TRANSACTION, CUSTOM_EVENTS, MOCKUP_MAP, PACKAGES_CONFIGURATION, screenLoader) {
        ...
        ctrl.initScreen = function () {
            uiApiModel.get(SCREEN_LAYOUTS.GROUP_OF_REQUESTS).then(function (data) {
                ...
                listsModel.get().then(function (data) {
                    ...
                    // if (group) {
                    //     groupId = group[0].requestGroup.id;
                    //     parseRequestList(group);
                    ...
                    function getGroupInfo(groupId) {
                        groupOfRequestsModel.getGroupInfo(groupId).then(function (data) {
                            ctrl.internalComments = data.payload.internalComments;
                            ctrl.externalComments = data.payload.externalComments;
         ...
         */
        function parseRequestList(requestList, remodel, skipSellerSorting) {
...
ctrl.setProductData = function(data, loc) {
	return data;
        ...
        ctrl.toggleEnergyContentTab = function (product) {
            if (ctrl.isEnergyCalculationRequired) {
                console.log(product);
                if (typeof ctrl.energyContentExpanded == "undefined") {
                    ctrl.energyContentExpanded = [];
                }
                ctrl.energyContentExpanded["prod" + product.id] = !ctrl.energyContentExpanded["prod" + product.id];
            } else {
        ...
        ctrl.saveComments = function (internalComments, externalComments) {
            groupOfRequestsModel.updateGroup(groupId, internalComments, externalComments, ctrl.quoteByDate, ctrl.quoteByTimezone, ctrl.quoteByCurrency);
        };
        ...
        ctrl.updatePhysicalSupplierForSellers = function (seller, newSupplier, location) {
        ...
        ctrl.calculateTotalAmountForProductsPerRequestperSeller = function (requestProducts, currLocation, seller) {
            ...
            $.each(requestProducts, function (k, product) {
                var productOffer = ctrl.getSellerProductOfferOnLocationRewrite(product, currLocation, seller.sellerCounterparty.id, seller);
                if (productOffer) {
        ...
        ctrl.initSupplierCardData = function (theLocation, seller, activeSellerCardTab) {
            ...
            } else if (!ctrl.sellerHasContact(seller, theLocation)) {
                ctrl.cannotViewSellerCard = true;
        ...
        $scope.$on("sendEmailRFQ", function (e, data) {
        	if (data == 'noReload') {
	            ctrl.sendRFQ(false);
        	} else {
	            ctrl.sendRFQ(true);
