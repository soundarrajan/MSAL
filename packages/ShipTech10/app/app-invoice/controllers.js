...
APP_INVOICE.controller('Controller_Invoice', ['$scope', '$rootScope', '$Api_Service', 'Factory_Invoice', '$state', '$location', '$q', '$compile', '$timeout', 'Factory_Master', '$listsCache', '$http', 'API', 'statusColors', '$tenantSettings', 'screenLoader', 'COMPONENT_TYPE_IDS', 'COST_TYPE_IDS', 'lookupModel', function($scope, $rootScope, $Api_Service, Factory_Invoice, $state, $location, $q, $compile, $timeout, Factory_Master, $listsCache, $http, API, statusColors, $tenantSettings, screenLoader, COMPONENT_TYPE_IDS,COST_TYPE_IDS, lookupModel) {
    ...
    $scope.invoiceConvertUom = function(type, rowIndex, formValues, oneTimeRun) {
        ...
        if ($scope.CM.type == 'product') {
            ...
            $scope.recalculatePercentAdditionalCosts(formValues);
        }
        ...
        $scope.recalculatePercentAdditionalCosts = function(formValues){
    		$.each(formValues.costDetails, function(ck,cv){
    			if (cv.costType.name == "Percent") {
    ...
    function isProductComponent(additionalCost) {
        ...
        if (vm.additionalCostTypes[additionalCost.costName.id].componentType) {
            additionalCost.isTaxComponent = !(vm.additionalCostTypes[additionalCost.costName.id].componentType.id === COMPONENT_TYPE_IDS.PRODUCT_COMPONENT);
