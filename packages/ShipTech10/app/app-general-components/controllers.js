        ...
        vm.get_formatter = function(name) {
            ...
            // ====== FORMATTERS ======
            {
                ...
                var confirmOrder = function(cellValue, options, rowObject) {
                    ...
                    return ' <a name="confirm" class="btn btn-outline btn-xs orange" ng-disabled="' + vm.canConfirm(rowObject) + "\"  ng-click=\"CLC.dataAction('confirmOrder', " + rowObject.order.id + ')" class="btn btn-outline btn-xs orange">Confirm</a>';
                };
                /**
                ...
                var reconfirmOrder = function(cellValue, options, rowObject) {
                    if (!rowObject.order) return;
                    return ' <a name="reconfirm" class="btn btn-outline btn-xs orange" ng-disabled="' + vm.canReconfirm(rowObject) + "\"  ng-click=\"CLC.dataAction('reconfirmOrder', " + rowObject.order.id + ')" class="btn btn-outline btn-xs orange">Reconfirm</a>';
                };
                ...
                var contract_planning_checkbox = function(cellValue, options, rowObject) {
                    var tpl = '<label class="mt-checkbox mt-checkbox-outline" style="padding-left: 5px;">' + '<input type="checkbox" class="contract_planning_checkbox" rowId="' + options.rowId + '" ng-model="selectContracts[' + options.rowId + ']"  ng-click="selectContractPlanningRow(' + options.rowId + ",selectContracts[" + options.rowId + '])"/><span></span></label>';
                ...
                var contract_planning_min_max_qty = function(cellValue, options, rowObject) {
                    ...
                    if (options.colModel.dataFrom == "base") {
                        if (rowObject.requestId == 0) {
                            // if requestId ==  0 -> min max qty editable
...
//                         '"
                    tpl +=
                        '<input ng-init="CLC.cpCtr[' +
                        options.rowId +
                        '] = CLC.getInitialContractValue('+options.rowId+');"  class="form-control no-right-border" typeahead-show-hint="true" typeahead-min-length="1" typeahead-append-to-body="true" typeahead-wait-ms="100" typeahead-on-select="CLC.changeCPRowModel($item, ' +
                        options.rowId +
                        "," +
                        columnKey +
                        ...
                        ')" ng-model="CLC.cpCtr[' +
                        options.rowId +
                        ']"  uib-typeahead="contract as contract.name for contract in CLC.getContractTypeaheadListCP(' +
                        options.rowId +
                        ')" />';
            ...
            // ng-click="triggerModal(\'general\', \'contractplanning_contractlist\', \'Contract\', \'Contract_\' + '+options.rowId+', \'\', \'\', \'Contract_\' + '+options.rowId+')"
                    tpl +='<i class="fa fa-search clickable form-control" data-toggle="modal" href="#selectContract" ng-click="openContractPopupInCP('+options.rowId+')" ><span hidden="true">&nbsp;</span></i>'
                ...
                var edit_request_link_from_delivery = function(cellValue, options, rowObject) {
                    ...
                    if (rowObject.requestId) {
                        var tpl = '  <a  href="#/edit-request/' + rowObject.requestId + '" style="width: calc(100% - 20px);"><span class="formatter edit_link" data-formatter-type="status" style="white-space:none" >' + cellValue + "</span></a>";
                    } else if (rowObject.voyageDetail){
                        if(rowObject.voyageDetail.request){
                            var tpl = '  <a  href="#/edit-request/' + rowObject.voyageDetail.request.id + '" style="width: calc(100% - 20px);"><span class="formatter edit_link" data-formatter-type="status" style="white-space:none" >' + cellValue + "</span></a>";
                        }else{
                            var tpl = '  <a  href="#/new-request/' + rowObject.voyageDetail.id + '" style="width: calc(100% - 20px);"><span class="formatter edit_link" data-formatter-type="status" style="white-space:none" >' + cellValue + "</span></a>";
                        }
                    } else {
                        var tpl = '  <a  style="width: calc(100% - 20px);"><span class="formatter edit_link" style="white-space:none" data-formatter-type="status">' + cellValue + "</span></a>";
                    }
                ...
                var edit_order_link = function(cellValue, options, rowObject) {
                    if (rowObject.order) {
                        var tpl = ' <a href="#/edit-order/' + rowObject.order.id + '" style="width: 100%"> <span class="formatter edit_link" data-formatter-type="link" style="white-space:none">' + rowObject.order.name + "</span></a>";
                    } else {
                ...
                var edit_request_link = function(cellValue, options, rowObject) {
                    ...
                    if (rowObject.requestGroupId) {
                        var tpl = ' <a href="#/group-of-requests/' + rowObject.requestGroupId + '" style="width: calc(100% + 30px);"> <span class="formatter edit_link" data-formatter-type="link" style="white-space:none">' + rowObject.request.id + "</span></a>";
                ...
                var groupOfRequests = function(cellValue, options, rowObject) {
                    var tpl = "";
                    if (cellValue) {
                        var tpl = ' <a href="#/group-of-requests/' + cellValue + '" style="width: calc(100% + 30px);"> <span class="formatter edit_link" data-formatter-type="link" style="white-space:none">' + cellValue + "</span></a>";
                    }
                    var element = tpl;
                    return element;
                };
        ...
        vm.editable_formatter = function(format, name) {
            ...
            var sap_date = function(cellValue, options, rowObject) {
                ...
                dateFormat = dateFormat.replace(/D/g, "d").replace(/Y/g, "y");
                formattedDate = $filter("date")(cellValue, dateFormat);
                var tpl = '<span ng-controller="SAPExportController as $ctrl"><a href="javascript:;" ng-click="$ctrl.sapExport(' + "'" + cellValue + "'" + ')"><span class="formatter">' + formattedDate + "</span></a></span>";
                return tpl;
            };
            var sap_download_link = function(cellValue, options, rowObject) {
                var tpl = '<a ng-controller="SAPExportController as $ctrl" href="javascript:;" ng-click="$ctrl.sapExport(' + "'" + rowObject.exportDate + "'" + ')"><span class="formatter">' + cellValue + "</span></a>";
                return tpl;
            };
        ...
        vm.changeCPRowModel = function(value, rowIdx, columnKey) {
